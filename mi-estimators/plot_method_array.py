...
for j in range(5):
    lambd =lambd_array[j]
    filename_ours  = 'Lip_features_mi_lam_'+str(lambd)+'_lip_g5_lip_5_D_500_gamma_5_gauss'#'Lip_RKHS_mi_lip_5_cubic'
    estimator = 'lip_features' #'lip_rkhs'
    critic = 'concat_lip_features'#'concat_lip_rkhs'
...
for j in range(5):
    ...
    mis_smooth = pd.Series(kl).ewm(span=EMA_SPAN).mean()
    plt.plot(mis_smooth, c=p1.get_color(), label='Ours')
    plt.plot(mi_schedule(x_limit//8), color='k', label='True MI')
    if j ==0:
        plt.ylabel('Lips =5, Lip-g = 2')
    axs[j+10].set_xticks(np.linspace(0,x_limit/8,6))
    plt.ylim(0, y_limit)
    plt.xlim(0, x_limit/8)

lambd_array =[0, 1e-05, 0.001, 0.1, 1]
for j in range(5):
    lambd =lambd_array[j]
    filename_ours  = 'Lip_features_mi_lam_'+str(lambd)+'_lip_g5_lip_3_D_500_gamma_5_gauss'#'Lip_RKHS_mi_lip_5_cubic'
    estimator = 'lip_features' #'lip_rkhs'
    critic = 'concat_lip_features'#'concat_lip_rkhs'
    ...
    plt.plot(mis_smooth, c=p1.get_color(), label='Ours')
    plt.plot(mi_true, color='k', label='True MI')
    axs[j + 15].set_xticks(np.linspace(0, x_limit, 6))
    if j == 0:
        plt.ylabel('Lips =3, Lip-g = 5')
    plt.ylim(0, y_limit)
    plt.xlim(0, x_limit)
...
for j in range(5):
    lambd =lambd_array[j]
    filename_ours  = 'Lip_features_mi_lam_'+str(lambd)+'_lip_g5_lip_10_D_500_gamma_5_gauss'#'Lip_RKHS_mi_lip_5_cubic'
    estimator = 'lip_features' #'lip_rkhs'
    critic = 'concat_lip_features'#'concat_lip_rkhs'
    ...
    plt.plot(mis_smooth, c=p1.get_color(), label='Ours')
    plt.plot(mi_true, color='k', label='True MI')
    axs[j + 20].set_xticks(np.linspace(0, x_limit, 6))
    if j ==0:
        plt.ylabel('Lips = 10, Lip-g = 5')
    plt.ylim(0, y_limit)
    plt.xlim(0, x_limit)
